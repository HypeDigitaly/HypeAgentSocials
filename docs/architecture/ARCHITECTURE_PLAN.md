...
The **curated inbox is a first-class source type**, not a paste box (D-09). It carries its own evidence class (human-asserted), its own retention rule, its own staleness flag, and its own honest label in the pack. Two consecutive misses escalate notification prominence rather than repeating an identical low-signal message (W2-01), and a **second escalation tier** fires after a configurable further-miss count: the axis label changes from *stale* to **abandoned**, the banner wording changes with it, and the pack stops describing the axis as merely late. Without that second tier the escalation mechanism becomes the thing it was designed to prevent — an identical alert repeating forever, one rung higher up the prominence scale.
...
| **Reddit** | Best-in-class ICP pain — IP, VD | P1, **human-gated** | Curated inbox | Weekly, ~30–60 min operator session | **Reddit Pro** (Reddit's own free business tool) plus manual thread curation; **no API, no scraping, ever, and no bulk export from Reddit Pro** — the output of the session is the operator's own written note, not a data extract (D-09, W2.5-2) | The ritual is skipped and the loss is invisible (W2-01) — mitigated by a per-source staleness flag, a digest banner naming the axis as degraded, packs labelled "pain axis: operator-fed", escalation after two consecutive misses and an *abandoned* tier beyond that (§2.2). **Separate open exposure: Reddit Pro is a different product from the ordinary-browser-session reading C7 assessed, with its own terms nobody has read** — a Phase-0 prerequisite (§17), on the same footing as the router's terms |
...
| **Provenance snapshot — verbatim part** | Always, alongside the durable part | The minimal quoted excerpt that triggered candidacy, and the canonical link to the individual post | **30 days**, on the same clock as curated-inbox verbatim text; on expiry it is replaced in the pack by the placeholder "excerpt expired — canonical key *K*" |
| **Curated-inbox verbatim text** | Yes | The operator's notes and quoted threads | 30 days, with keyed author hashes (OD-15 recommendation) |
...
| **Short video / Reel / Short / slideshow** | **Full playthrough, and this is the bottleneck** | Motion integrity, hook in the first three seconds, caption legibility at mobile scale, audio prosody and timing, claim verifiability, clean loop or ending, burned-in disclosure present from the first second | **20–30 minutes per finished video** (A1) |
...
- The digest of the **adopting** run carries a one-line cross-reference naming which earlier pack it amended, and the amended pack's own digest gains an "amended on *date*" line. The operator is never expected to notice a folder changing underneath them.
...
| Tier | Media, per two-language topic pack | Text, per two-language topic pack | What $50 buys (media + text) |
|---|---|---|---|
...
**The text column is not a rounding error at run scale, which is why it is now a column.** A two-language topic pack runs 140,000–450,000 tokens depending entirely on whether regenerate loops fire (C5 §6), and a pack *run* carries roughly five topics, so a single run is on the order of 0.7–2.25 million tokens and three pack runs a week is roughly 9–27 million tokens a month. At mid-tier blended text pricing that is the same order of magnitude as the media forecast, not a fraction of it — and it was entirely absent from this table. It stays expressed as a range read from the registry rather than as a number, on the same principle as every media figure: the forecast is computed at run time from price snapshots with the snapshot date shown, and no figure here is normative.
...
| **Per-stage call ceiling** | Calls, per stage, per run | The ceilings §1.5 asserted existed. One per stage: ranking (bounded by candidate count × the N-1/N-2 nodes), copy generation (bounded by asset count × the drafting nodes), each gate (bounded by artifact count × its node) | The stage completes on what it has and the remainder degrades on that stage's own named path — unranked candidates are dropped with a reason, ungenerated assets become plan-only, un-gated artifacts fail closed (§11.3) |
...
**What the forecast shows.** Two lines, never one blended figure: media forecast and text forecast, each with its price-snapshot date. The text forecast is computed from the planned artifact count × the gate stack's per-artifact call profile × the route's token ceilings, which is exactly the arithmetic that makes the number checkable rather than a guess — and is why the artifact count is the honest driver to look at when the forecast surprises someone.
...
The canonical per-asset order, stated once:
    ...
    generate
      ...
      -> CLAIM GATE pass 2    final, immutable, on the exact packed TEXT bytes
      |     |
      |     +-- if pass 2 REPAIRS the text (bounded regenerate or
      |         downgrade repair), the text has been rewritten, so:
      |         RE-ENTER -> VOICE GATE -> SPIN post-check -> CLAIM pass 2
      |         (counted against the single per-artifact repair ceiling;
      |          on exhaustion take the downgrade-repair variant, never
      |          another lap)
      v
      -> PLATFORM GATE        "does it fit this destination's hard limits?"
      -> (media only) COST GATE -> media generation -> assembly
      -> POST-ASSEMBLY OVERLAY CLAIM PASS   deterministic classes over the
         composed on-screen strings, because composition happens after
         claim pass 2 closes (§4.4)
...
**Recorded.** The **review-decision store** (canonical ledger, §8.6) holds one reason-coded approve/reject/partial decision per asset (and, where useful, per topic or per pack as a whole), keyed by run id and asset id, with attempt history retained rather than overwritten. Each decision additionally carries **the identity of the approving person** and a per-asset **editorial-responsibility-held-by** value — the second condition of the human-editorial-review carve-out (§3.3), and the field that makes "a named natural or legal person holds editorial responsibility" evidenced rather than asserted. It also carries any **pack-amendment records** written when a later run completes a pending master against this run id (§4.7a), so an amended pack's review history stays in one place. Two input mechanisms write into this same store — they are not two different approval models, only two ways of expressing the same decision: editing a **decision file** that accompanies the run digest (§12), or issuing an interactive console command that references the run id. A rejection is always reason-coded, both because that discipline is what makes the learning loop in §12 trustworthy and because "the topic was wrong" must never be conflated with "I had enough content this week" when feedback is later aggregated.
...
- a **cost forecast**, prominent, decomposed per topic and **shown as two lines — media and text — never blended**, each reading from the model registry's current price snapshots with its snapshot date rather than any hard-coded figure (so the forecast stays honest as provider prices drift — SYNTHESIS §3.10 corrects an earlier illustrative-figures approach in favor of this; §5.4a adds the text line);
...
- degraded-source banners: which sources were unavailable this run and how that is expected to affect coverage (per-source degrade notes, §8.10), **including the *stale payload suspected* reason** (§2.2), plus the curated-inbox staleness banner when the Reddit-curation ritual has been missed (W2-01, §8.12) and its *abandoned* escalation beyond that;
...
| **OD-16** | **Trend-intelligence vendor adoption** — the vendor being **Virlo.ai** at its Starter tier (~$49/mo), approved at W2.5-3 but trial-gated | (a) Adopt Virlo after a passing trial; (b) fall back to **Shortimize**, the named same-category vendor, at the same gate; (c) accept the designed degraded state — a monthly manual browse of the **TikTok Creative Center** top-ads gallery | **(a), and the trial must answer three questions before any subscription renews**: is full programmatic MCP/API access genuinely included at the Starter tier (Virlo's own guide and pricing page conflict, and this single fact gates adoption); does a niche monitor in this subject area beat what the free portfolio already surfaces; and what are the unpublished rate limits | Operator, at the end of the one-week trial in Phase 0. A failed trial moves to (b), then (c) — never to a silent gap |
...
**Five further acceptance criteria, each testing something that was previously asserted rather than checked:**
