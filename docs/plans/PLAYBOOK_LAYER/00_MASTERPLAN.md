...
| **C-4** | `02` makes F-B/F-C/F-E legitimately emptiable; `03` requires substitution, never exemption | **Binding wording is stated in §4a.** `04_RECONCILIATION.md` may restate this ruling for consistency across documents but may not re-decide it. The substitution floor and negative-capability requirement are prerequisites for Amendment A and must not drift across waves. |
...
## 4a. Amendment A prerequisite rulings

These rulings from C-4 and C-12 are stated in full here to establish Amendment A's severability from Amendment B. `04_RECONCILIATION.md` may restate these rulings for consistency and completeness, but may not re-decide them.

**C-4: External-verifier substitution floor**

Where F-B, F-C or F-E are legitimately empty, their descriptive content is replaced by a grounded substitute. The substitution floor is raised beyond `03`'s non-triviality test, which admitted *"an explicit, dated operator attestation"* as sufficient — but tenant self-assertion is precisely what `02` §2.1 spends its argument proving cannot ground a claim.

**A substitute grounding any descriptive-or-stronger statement class requires an external verifier.** An external verifier is one of: a site check (visiting the tenant's domain, confirming it is live and connected, verifying a claim-supporting element exists in person — not a HTTP status, but a visible page element matching the claim's specificity), a resolving URL (a link the tenant supplies that returns HTML rendering a claim-supporting page — this is not a link checker, but a return to a page containing words or images substantiating the claim), or a third-party record (testimony from an independent source carrying audit authority or transactional grounding — a consulting firm's published case study, a review platform's substantiated rating, a news article).

**Operator attestation suffices only for statement classes whose complete absence cannot produce a claim.** This is narrower than the original scope. Where a class is claim-bearing at all, operator word alone is insufficient.

**A substitute for F-B (product capability claims) must carry a negative-capability field**, or the substitution is rejected and check class 6 (fact-grounding) degrades to blocking all descriptive statements for that tenant. The negative-capability field is a statement the operator makes about what the product does *not* do — a boundary that constrains overgeneralisation, used in enforcement to detect claims that cross a line the operator has drawn. Without it, count preservation becomes numerology, not safety.

**C-12: Deterministic-only depiction check in v1**
...
**The substantive visual comparison — is this item depicted as it is actually sold, with no alterations that mislead — is the human attestation's job.** That attestation is required by `02` §2.5 and the review pack must show the side-by-side. The engine contributes only the gate: a provenance record and a coherent reference exist, preventing orphaned images with no grounding claim. The visual integrity of the depiction policy belongs to the operator's attestation and theme-level governance, not to a class-12 model call.

---

## 5. Amendment A — safety, before Stage-5 approval

**Rationale.** P-10 and P-11 apply to every tenant and depend on nothing in the playbook layer — the Prohibited-Outcome Gate sits *beneath* it by design and check class 12 is base-pack and universal. Bundling them into a multi-wave generalisation programme means the operator either approves a design with two known safety defects or waits weeks for a decision unrelated to them. Neither is acceptable. The P-1 hard-failure fix rides along because it is two paragraphs and closes a "may never be scheduled" defect outright. Review established that Amendment A was not severable as first written, because tasks A2 and A3 depended on rulings whose binding text lived in Amendment B's Wave-0 file; §4a relocates that text so Amendment A stands alone, which is the entire justification for the A/B split.
...
| **A2** | `legal-advisor` *(after A1, same owner-domain, sequential — shares the gate-order statement)* | `docs/architecture/playbook/A2_depiction.md` | Check class 12 **deterministic-only per §4a**; fact class F-W reference imagery; the three-value depiction policy with the engine default at the safe end; the human attestation and the mandatory side-by-side the review pack must show. Note explicitly that Policy A as engine floor **removes an existing capability from theme #1** (its own product-dashboard renders) — that is a permitted change only if called out as intended. |
| **A3** | `api-designer` | `docs/architecture/playbook/A3_emptiability.md` | F-B/F-C/F-E legitimate emptiability with **§4a's external-verifier** substitution floor and the negative-capability requirement; F-F's separate safe-default fix. Nothing about playbooks. |
...
**A consequence review drew out and the plan accepts:** six genres × two languages is twelve calibration cells, each needing a golden set and — because the Czech rhythm band is measured from a structural corpus — its own per-genre Czech corpus. At one tenant and a few runs a week those cells will not fill for years, and `03`'s own sample-size argument ("a ceiling computed on four artifacts is noise dressed as a control") condemns them. **v1 therefore registers all genres but calibrates two**: analytical-B2B, plus whichever playbook #2 needs. The rest are design-complete with flag-rate ceilings recorded inactive — the mechanism already used for a new language, applied to a new genre. The per-genre Czech calibration corpus is added to PB-OD-7's cost.

**No phase currently delivers a per-genre corpus or per-genre golden set.** These are named as required inputs to calibration, but no wave author is charged with producing them and no build phase gates on their completion. This is a defect that must be corrected: per-genre Czech calibration corpus and per-genre golden set must become explicit deliverables with the same do-not-start rule the four existing calibration artefacts carry — otherwise a registered genre's flag-rate ceiling stays permanently inactive and Tier 2 becomes a pure relaxation surface with no control on it.
